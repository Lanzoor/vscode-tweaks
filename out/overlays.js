...
class Overlay {
    ...
    bindAnimations() {
        if (!this.animated || this.animationsBound)
            return;
...
let keyOverlayInterval = null;
function updateKeyOverlay() {
    if (keyOverlayInterval)
        clearInterval(keyOverlayInterval);
...
function loop() {
    ...
    if (now - lastUpdateTime >= 250) {
        const fps = Number(((frameCount * 1000) / (now - lastUpdateTime)).toFixed(2));
